...
const CustomerDetails = () => {
  const { customerDetails, activeSection } = useCartUserInfoContext();
  ...
  const handleClickEdit = () => {
    dispatch({
      ...
      payload: {
        newSection: CART_USER_INFO_SECTIONS.CUSTOMER_DETAILS,
      },
    });
  };
      ...
      {activeSection === CART_USER_INFO_SECTIONS.CUSTOMER_DETAILS ? (
                ...
                <input
                  ...
                  className={clsx(
                    "w-full h-11 px-3 border border-[#CCCCCC] rounded-sm focus:outline-none focus:border-black transition-colors",
                    {
                      "text-sm":
                        formField.label !==
                          CUSTOMER_DETAILS_FIELDS.FIRST_NAME &&
        ...
        <div>
          ...
          <div className="flex gap-1">
            ...
            <p>{customerDetails.city && `${customerDetails.city},`}</p>
            <p>{customerDetails.countryOrRegion}</p>
          </div>
        </div>
      )}
    </div>
